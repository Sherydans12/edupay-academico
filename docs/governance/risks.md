...
| Identity contract is not available or changes late | Blocks authentication and membership behavior | High | Reconciled baseline in ADR-0009; use contract fakes, JWKS/claim contract tests, and require reviewed ADR changes; isolate adapter |
...
| Over-broad tenant-admin or system-admin permissions | Unauthorized access | Medium | Reconciled MVP policy matrix, no automatic system-admin tenant access, explicit elevated-action audit, and negative tests |
