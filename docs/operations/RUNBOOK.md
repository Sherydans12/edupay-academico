...
2. Partir de la rama publicada `codex/production-stable-baseline` del repositorio
   correspondiente para trabajo sobre la base funcional cerrada. Crear otra
   rama `codex/<mejora>` y otro worktree. Mantener la baseline limpia.
3. En Académico, esa baseline incluye el frontend 4f5ad28 y documentación:
   **main no era el código desplegado al cierre**. API y workers permanecen
   pinned a b2f489f. No promover toda una rama monorepo a todos los recursos.
