...
describe.runIf(testDatabaseUrl)(
  'Storage and submissions (PostgreSQL e2e)',
  () => {
    ...
    it('uses metadata-only intents, bounded multipart transfer, quota conversion, and private tenant-local deduplication', async () => {
      ...
      const usageWithReservation = (
        await api(setup.teacherToken).get('/api/v1/storage/usage').expect(200)
      ).body;
      expect(storageUsageSchema.parse(usageWithReservation)).toEqual(
        usageWithReservation,
      );
      expect(usageWithReservation.reservedBytes).toBe(bytes.length);
      expect(usageWithReservation.temporaryOrStagedBytes).toBe(0);
      const reservedBeforeTransfer =
      ...
      await api(setup.studentToken)
        .get(`/api/v1/files/${first.id}/download`)
        .expect(403);
      await prisma.storedBlob.update({
        where: { tenantId_id: { tenantId: 'storage-a', id: firstBlob.id } },
      ...
      expect(
        await prisma.storedBlob.count({ where: { tenantId: 'storage-a' } }),
      ).toBe(1);
      expect(
        await prisma.fileObject.count({ where: { tenantId: 'storage-a' } }),
      ).toBe(2);
      const usageAfterDedup = (
        await api(setup.teacherToken).get('/api/v1/storage/usage').expect(200)
      ).body;
      expect(storageUsageSchema.parse(usageAfterDedup)).toEqual(
        usageAfterDedup,
      );
      ...
      const otherActor = await token('storage-a', 'unassigned-teacher', [
        'TEACHER',
      ]);
    ...
    it('expires intents, rejects oversized multipart payloads, and cleans failed transfer staging', async () => {
      ...
      expect(
        (
          await prisma.uploadIntent.findUniqueOrThrow({
            where: {
              tenantId_id: { tenantId: 'hardening-a', id: oversized.id },
            },
          })
        ).status,
      ).toBe('FAILED');
      ...
      expect(
        (
          await prisma.uploadIntent.findUniqueOrThrow({
            where: { tenantId_id: { tenantId: 'hardening-a', id: invalid.id } },
          })
        ).status,
      ).toBe('FAILED');
      expect(
      ...
      expect(
        (
          await prisma.uploadIntent.findUniqueOrThrow({
            where: {
              tenantId_id: { tenantId: 'hardening-a', id: infected.id },
            },
          })
        ).status,
      ).toBe('FAILED');
      expect(
        await prisma.storedBlob.count({ where: { tenantId: 'hardening-a' } }),
      ).toBe(0);
      expect(
        await prisma.fileObject.count({ where: { tenantId: 'hardening-a' } }),
      ).toBe(0);

      const timedOutBytes = Buffer.concat([
        Buffer.from('%PDF-'),
        FAKE_TIMEOUT_MARKER,
      ]);
    ...
    it('accepts only separately finalized authorized student files and preserves immutable revision semantics', async () => {
      ...
      const firstFile = await uploadStudentFile(
        setup.studentToken,
        setup.item.id,
        'first.pdf',
        pdf('first'),
      );
      const secondFile = await uploadStudentFile(
        setup.studentToken,
        setup.item.id,
        'second.pdf',
        pdf('second'),
      ...
      const createdResponse = await api(setup.studentToken)
        .post(`/api/v1/learning-items/${setup.item.id}/submission`)
        .send({
          fileObjectIds: [firstFile.id, secondFile.id],
      ...
      const otherStudent = await post(admin, '/api/v1/students', {
        firstName: 'Other',
        lastName: 'Student',
      });
      await prisma.student.update({
        where: {
          tenantId_id: { tenantId: 'submission-a', id: otherStudent.id },
        },
      ...
      const otherStudentToken = await token('submission-a', 'other-student', [
        'STUDENT',
      ]);
      ...
      await api(setup.teacherToken)
        ...
        .send({
          ...
          comment: 'Please correct the document.',
        })
        .expect(201);
      expect(
        await prisma.notificationEvent.count({
      ...
      const revisedFile = await uploadStudentFile(
        setup.studentToken,
        setup.item.id,
        'corrected.pdf',
        pdf('corrected'),
      ...
      expect(revised.revisions[0].files[0].id).not.toBe(
        revised.revisions[1].files[0].id,
      );
      expect(
        await prisma.notificationEvent.count({
          where: {
    ...
    async function createDeliverable(
      ...
      const course = await post(admin, '/api/v1/courses', {
        ...
        label: `Course ${tenantId}`,
        status: 'ACTIVE',
      });
      const subject = await post(admin, '/api/v1/subjects', {
        name: `Subject ${tenantId}`,
      });
      const courseSubject = await post(admin, '/api/v1/course-subjects', {
        courseId: course.id,
      ...
      await prisma.teacher.update({
        ...
        data: { identityUserId: teacherIdentity },
      });
      await prisma.student.update({
        where: { tenantId_id: { tenantId, id: student.id } },
        data: { identityUserId: studentIdentity },
      });
