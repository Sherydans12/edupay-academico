...
describe.runIf(testDatabaseUrl)(
  'Academic notifications (PostgreSQL e2e)',
  () => {
    ...
    it('creates tenant-isolated publication intents, handles scheduled delivery once, and exposes current-user API state', async () => {
      ...
      const directSubject = await post(admin, '/api/v1/subjects', {
        name: 'Apoyo',
      });
      ...
      const teacherToken = await token(tenantId, 'teacher-notifications', [
        'TEACHER',
      ]);
      const studentToken = await token(tenantId, 'student-enrolled', [
        'STUDENT',
      ]);
      ...
      await post(
        teacherToken,
        `/api/v1/learning-items/${assignment.id}/publish`,
        {},
      );
      ...
      await patch(teacherToken, `/api/v1/learning-units/${directUnit.id}`, {
        status: 'ACTIVE',
      });
      const directAssignment = await post(
        teacherToken,
        `/api/v1/learning-units/${directUnit.id}/items`,
        {
          type: 'ASSIGNMENT',
      ...
      await post(
        teacherToken,
        `/api/v1/learning-items/${directAssignment.id}/publish`,
        {},
      );

      expect(
        await prisma.notificationEvent.count({ where: { tenantId } }),
      ).toBe(2);
      expect(
      ...
      expect(
        await prisma.inAppNotification.count({ where: { tenantId } }),
      ).toBe(2);
      expect(
        (
      ...
      const page = await api(studentToken)
      ...
      const emailBefore = await prisma.notificationDelivery.count({
        where: { tenantId, channel: 'EMAIL', status: 'PENDING' },
      });
      ...
      expect(
        concurrentRuns.reduce((total, result) => total + result.claimed, 0),
      ).toBe(2);
      expect(
        await prisma.notificationDelivery.count({
          where: { tenantId, channel: 'EMAIL', status: 'DELIVERED' },
        }),
      ).toBe(2);
      ...
      await post(
        teacherToken,
        `/api/v1/learning-items/${scheduled.id}/schedule`,
        {
      ...
      expect(
        await prisma.notificationEvent.count({
          where: { tenantId, aggregateId: scheduled.id },
        }),
      ).toBe(0);
      ...
      expect(
        await prisma.notificationDelivery.count({
          where: { tenantId, event: { aggregateId: scheduled.id } },
        }),
      ).toBe(2);
    });
