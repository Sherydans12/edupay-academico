...
const resolvedPersonnelSchema = responseSchema.extend({
  institutionalUsername: z.string().min(1).max(128),
});
...
export class DieIdentityMembershipVerifier {
  ...
  async verify(
    ...
    if (
      ...
      parsed.data.roles.some((role) => role === 'STUDENT' || role === 'GUARDIAN') ||
      !parsed.data.roles.some((role) =>
        ['STAFF', 'TEACHER', 'TENANT_ADMIN'].includes(role),
      )
    ) {
      throw new ForbiddenException(
        'The requested department member is not eligible.',
  ...
  ): Promise<VerifiedDieTargetMembership & { readonly institutionalUsername: string }> {
    let response: unknown;
    try {
      response = await this.client.post(
      ...
      if (error instanceof IdentityInternalHttpResponseError && (error.status === 403 || error.status === 404)) {
        throw new ForbiddenException('The requested department member is not eligible.');
      }
      throw new ServiceUnavailableException('Identity verification is temporarily unavailable.');
    }
    const parsed = resolvedPersonnelSchema.safeParse(response);
    if (
      !parsed.success ||
      parsed.data.tenantId !== context.tenant.tenantId ||
      parsed.data.roles.some((role) => role === 'STUDENT' || role === 'GUARDIAN') ||
      !parsed.data.roles.some((role) => ['STAFF', 'TEACHER', 'TENANT_ADMIN'].includes(role))
    ) {
      throw new ForbiddenException('The requested department member is not eligible.');
    }
    return parsed.data;
