...
const subject = {
  id: subjectId,
  courseId: '00000000-0000-4000-8000-000000000109',
  subjectId: '00000000-0000-4000-8000-000000000110',
...
const item = {
  ...
  publishedByIdentityUserId: 'teacher',
  dueAt: '2026-08-08T10:00:00+00:00',
  createdByIdentityUserId: 'teacher',
...
const policy = {
  ...
  globalQuotaBytes: 20_000_000_000,
  tenantQuotaBytes: 20_000_000_000,
  initialOperationalTimezone: 'America/Santiago' as const,
};
const file = {
  id: fileId,
...
function revision(number: number, late = true) {
  return {
    id: number === 1 ? revisionId : '00000000-0000-4000-8000-000000000109',
    revisionNumber: number,
    studentComment: number === 1 ? 'Primera entrega' : 'Corregida',
    submittedAt: timestamp,
    effectiveDueAt: '2026-08-08T10:00:00+00:00',
    isLate: late,
...
function submission(
  status: 'SUBMITTED' | 'REVIEWED' | 'CHANGES_REQUESTED' = 'SUBMITTED',
) {
  return {
    id: submissionId,
    studentId,
...
describe('student storage and submission workflow', () => {
  it('uploads multiple files independently before the first submission and renders server late state/history', async () => {
    ...
    const completeUploadIntent = vi.fn(async (_intent, selected: File) =>
      selected.name === 'trabajo.pdf' ? file : secondFile,
    );
    ...
    fireEvent.change(input, {
      target: {
        files: [
          new File(['pdf'], 'trabajo.pdf', { type: 'application/pdf' }),
          new File(['pdf'], 'anexos.pdf', { type: 'application/pdf' }),
        ],
      },
    });
    fireEvent.change(screen.getByLabelText('Comentario opcional'), {
      target: { value: 'Revisa la introducción.' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Enviar trabajo' }));

    await waitFor(() =>
      expect(submitLearningItem).toHaveBeenCalledWith(itemId, {
        fileObjectIds: [fileId, secondFileId],
        studentComment: 'Revisa la introducción.',
      }),
    );
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Descargar trabajo.pdf' }),
    );
  ...
  it('shows an individual upload failure and retries only the failed file', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Enviar trabajo' }));
    expect(
      await screen.findByRole('button', { name: 'Reintentar anexos.pdf' }),
    ).toBeTruthy();
    fireEvent.click(
      screen.getByRole('button', { name: 'Reintentar anexos.pdf' }),
    );
    await waitFor(() =>
      expect(
        (
  ...
  it('resubmits only after CHANGES_REQUESTED and blocks free resubmission after REVIEWED', async () => {
    ...
    const submitSubmissionRevision = vi.fn(async () => ({
      ...changes,
      revisions: [revision(1), revision(2, false)],
    }));
    const api = client({
      getStudentContextSubjects: vi.fn(async () => [subject]),
      getLearningItem: vi.fn(async () => item),
      getOwnSubmission: vi.fn(async () => changes),
      getStoragePolicy: vi.fn(async () => policy),
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Enviar nueva revisión' }),
    );
    await waitFor(() =>
      expect(submitSubmissionRevision).toHaveBeenCalledWith(
        submissionId,
        expect.objectContaining({ fileObjectIds: [fileId] }),
      ),
    );
    ...
    render(
      <StudentAssignmentScreen
        api={client({
          getStudentContextSubjects: vi.fn(async () => [subject]),
          getLearningItem: vi.fn(async () => item),
          getOwnSubmission: vi.fn(async () => submission('REVIEWED')),
          getStoragePolicy: vi.fn(async () => policy),
        })}
        courseSubjectId={subjectId}
        learningItemId={itemId}
...
describe('teacher review and attachment workflow', () => {
  ...
  it('supports comment, reviewed, changes-requested actions and authorized file download', async () => {
    const reviewSubmissionRevision = vi.fn(async (_revisionId, input) => ({
      ...submission(
    ...
    fireEvent.change(screen.getByLabelText('Comentario para el estudiante'), {
      target: { value: 'Corrige la conclusión.' },
    });
    fireEvent.click(screen.getByRole('button', { name: /comentar/i }));
    await waitFor(() =>
      expect(reviewSubmissionRevision).toHaveBeenCalledWith(revisionId, {
        action: 'COMMENTED',
        comment: 'Corrige la conclusión.',
      }),
    );
  ...
  it('uploads learning attachments with the item category', async () => {
    ...
    const api = client({
      getTeacherContextSubjects: vi.fn(async () => [subject]),
      getLearningRoute: vi.fn(async () => route),
      listLearningAttachments: vi.fn(async () => []),
      getStoragePolicy: vi.fn(async () => policy),
      getStorageUsage: vi.fn(async () => ({
        ...
        byCategory: [],
        logicalUsedBytes: 10,
        physicalBlobBytes: 10,
        temporaryOrStagedBytes: 0,
        physicalStorageTotalBytes: null,
        physicalStorageFreeBytes: null,
        reconciliationStatus: 'CONSISTENT' as const,
      })),
      createUploadIntent,
      completeUploadIntent,
    });
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: `Gestionar archivos de ${item.title}`,
      }),
    );
    const input = await screen.findByLabelText('Selecciona tus archivos');
    fireEvent.change(input, {
      target: {
        files: [new File(['pdf'], 'guia.pdf', { type: 'application/pdf' })],
      },
    });
  ...
  it('blocks new uploads at full quota while preserving download access', async () => {
    ...
    const api = client({
      getTeacherContextSubjects: vi.fn(async () => [subject]),
      getLearningRoute: vi.fn(async () => route),
      listLearningAttachments: vi.fn(async () => [file]),
      getStoragePolicy: vi.fn(async () => policy),
      getStorageUsage: vi.fn(async () => ({
        tenantId: 'tenant-a',
        quotaBytes: 20,
    ...
    expect(
      screen.getByText(/no se pueden subir archivos nuevos/i),
    ).toBeTruthy();
    const input = screen.getByLabelText('Selecciona tus archivos');
