...
function initialUnit(unit?: LearningUnitWithItems): UnitDraft {
  const draft = { description: unit?.description ?? '', endAt: learningInstantToDateTimeLocal(unit?.endAt ?? null), startAt: learningInstantToDateTimeLocal(unit?.startAt ?? null), title: unit?.title ?? '' };
  return unit ? { ...draft, id: unit.id } : draft;
}

function initialItem(item?: LearningItem): ItemDraft {
  const draft = { body: item?.body ?? '', content: item?.content ?? '', description: item?.description ?? '', dueAt: learningInstantToDateTimeLocal(item?.dueAt ?? null), instructions: item?.instructions ?? '', title: item?.title ?? '', type: item?.type ?? 'MATERIAL' as const };
...
export function TeacherSubjectScreen({ api, courseSubjectId, session = demoSessions.teacher }: { api?: AcademicApiClient; courseSubjectId?: string; session?: TrustedCurrentSession }) {
  ...
  function itemInput(values: ItemDraft, confirmSensitiveChange = false) {
    ...
    return { body: values.type === 'ANNOUNCEMENT' ? values.body || undefined : undefined, content: values.type === 'MATERIAL' ? values.content || undefined : undefined, confirmSensitiveChange, description: values.description || undefined, dueAt: deliverable ? learningDateTimeLocalToInstant(values.dueAt) : undefined, instructions: deliverable ? values.instructions || undefined : undefined, title: values.title, type: values.type };
  }
  async function saveUnit() {
    if (!unitDraft || !data.selected) return;
    const courseSubjectId = data.selected.id;
    await run(async () => { if (unitDraft.id) await client.updateLearningUnit(unitDraft.id, { description: unitDraft.description || null, endAt: learningDateTimeLocalToInstant(unitDraft.endAt) ?? null, startAt: learningDateTimeLocalToInstant(unitDraft.startAt) ?? null, title: unitDraft.title }); else await client.createLearningUnit({ courseSubjectId, description: unitDraft.description || undefined, endAt: learningDateTimeLocalToInstant(unitDraft.endAt), sortOrder: 0, startAt: learningDateTimeLocalToInstant(unitDraft.startAt), title: unitDraft.title }); setUnitDraft(null); });
  }
