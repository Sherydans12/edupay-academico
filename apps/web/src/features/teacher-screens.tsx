...
import { STORAGE_SUBMISSION_SEAM_STATE } from '@/api/storage-submission-seam';
...
function LearningActions({ item, itemIndex, itemCount, onArchive, onEdit, onMove, onPublish, onSchedule }: { item: LearningItem; itemIndex: number; itemCount: number; onArchive: () => void; onEdit: () => void; onMove: (direction: -1 | 1) => void; onPublish: () => void; onSchedule: () => void }) {
  return <div className="learning-action-group"><Button aria-label={`Subir ${item.title}`} disabled={itemIndex === 0} onClick={() => onMove(-1)} size="icon" title="Subir" variant="ghost"><Icon name="chevron-down" className="rotate-180" /></Button><Button aria-label={`Bajar ${item.title}`} disabled={itemIndex === itemCount - 1} onClick={() => onMove(1)} size="icon" title="Bajar" variant="ghost"><Icon name="chevron-down" /></Button><Button aria-label={`Editar ${item.title}`} onClick={onEdit} size="sm" variant="secondary"><Icon name="settings" />Editar</Button>{item.publicationStatus === 'DRAFT' ? <><Button onClick={onPublish} size="sm">Publicar</Button><Button onClick={onSchedule} size="sm" variant="accent"><Icon name="calendar" />Programar</Button></> : null}{item.publicationStatus === 'SCHEDULED' ? <Button onClick={onPublish} size="sm">Publicar ahora</Button> : null}{item.publicationStatus !== 'ARCHIVED' ? <Button aria-label={`Archivar ${item.title}`} onClick={onArchive} size="icon" title="Archivar" variant="ghost"><Icon name="archive" /></Button> : null}</div>;
}
...
export function TeacherDashboardScreen({ api, session = demoSessions.teacher }: { api?: AcademicApiClient; session?: TrustedCurrentSession }) {
  ...
  return <AppShell dataMode="real" session={currentSession}><PageHeading action={<Link className="button-link button-link--primary" href="/docente/asignaturas"><Icon name="book" />Ver contenido</Link>} description="Tus CourseSubjects autorizados y sus rutas de aprendizaje reales." title={`Buenos días, ${currentSession.displayName.split(' ')[0]}`} /><TeacherDataState error={data.error} loading={data.loading} onRetry={() => void data.load()}><div className="compact-stats"><CompactStat icon="book" label="CourseSubjects asignados" value={String(data.subjects.length)} /><CompactStat icon="review" label="Entregas" value="Pendiente" /><CompactStat icon="calendar" label="Calendario" value="Pendiente" /></div><div className="teacher-dashboard-grid"><section aria-labelledby="teacher-route-title" className="content-section teacher-review-queue"><div className="section-heading"><div><h2 id="teacher-route-title">Contenido autorizado</h2><p>Abre un espacio para organizar sus unidades e ítems.</p></div></div>{data.subjects.length ? <div className="subject-grid subject-grid--overview">{data.subjects.map((subject, index) => <SubjectCard key={subject.id} subject={subjectCard(subject, index, 'teacher')} />)}</div> : <EmptyState icon={<Icon name="book" />} title="No tienes CourseSubjects asignados" description="Un administrador debe asignarte a un CourseSubject activo." />}</section><aside className="week-plan"><h2>Entregas y revisiones</h2><p>La información de archivos, entregas, historial y revisión permanecerá aquí cuando Storage/Submissions publique sus contratos.</p><Badge tone="info">{STORAGE_SUBMISSION_SEAM_STATE === 'NOT_CONNECTED' ? 'Integración pendiente' : 'Conectado'}</Badge></aside></div></TeacherDataState></AppShell>;
}
...
export function SubmissionReviewScreen({ session = demoSessions.teacher }: { session?: TrustedCurrentSession }) {
  const currentSession = useTrustedCurrentSession(session).session;
  return <AppShell dataMode="real" session={currentSession}><PageHeading description="La revisión se habilitará cuando Storage/Submissions publique sus contratos y estados aceptados." title="Revisión de entrega" /><Alert title="Integración pendiente" tone="info">No se muestran estudiantes, archivos ni estados sintéticos. Esta pantalla queda como punto de entrada para la próxima integración.</Alert></AppShell>;
}

export function TeacherReviewsScreen({ session = demoSessions.teacher }: { session?: TrustedCurrentSession }) {
  const currentSession = useTrustedCurrentSession(session).session;
  return <AppShell dataMode="real" session={currentSession}><PageHeading description="Los registros de entregas y revisiones pertenecerán al bounded context Storage/Submissions." title="Revisiones" /><EmptyState icon={<Icon name="review" />} title="Revisiones aún no conectadas" description="Cuando se integren los contratos, aquí aparecerán únicamente entregas autorizadas para tus CourseSubjects." /></AppShell>;
}
