...
function authValue(
  overrides: Partial<IdentitySessionContextValue> = {},
): IdentitySessionContextValue {
  return {
    status: 'unauthenticated',
    session: null,
    memberships: [],
    login: vi.fn(async () => studentSession),
    logout: vi.fn(),
    refresh: vi.fn(),
...
beforeEach(() => {
  vi.stubEnv(
    ...
    'https://academico.example.test/api/v1',
  );
...
describe('login', () => {
  it('uses real session login and redirects a student to the student workspace', async () => {
    ...
    fireEvent.change(
      screen.getByLabelText('Usuario institucional o correo verificado'),
  ...
  it('shows the same generic invalid-credential response', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Entrar' }));
    expect(
      await screen.findByText(/No pudimos verificar las credenciales/i),
    ).toBeTruthy();
  });
  ...
  it('offers bounded membership choices and retries with the selected tenant handle', async () => {
    ...
    fireEvent.change(
      screen.getByLabelText('Usuario institucional o correo verificado'),
      { target: { value: 'sofia@example.test' } },
    );
    fireEvent.change(screen.getByLabelText('Contraseña'), {
      target: { value: 'secret' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Entrar' }));
    const tenantChoice = await screen.findByRole('button', {
      name: /segundo-colegio/i,
    });
    fireEvent.click(tenantChoice);

    await waitFor(() =>
      expect(login).toHaveBeenLastCalledWith(
        expect.objectContaining({ tenantHandle: 'segundo-colegio' }),
      ),
    );
...
describe('activation and recovery', () => {
  it('accepts an invitation token, clears secret fields, and removes the token from history', async () => {
    navigation.pathname = '/activate';
    navigation.search = 'token=invitation-secret';
    const fetchMock = vi.fn<typeof fetch>(async () =>
      json({ membershipId: 'membership-1', status: 'ACTIVE' }),
    );
    vi.stubGlobal('fetch', fetchMock);
    const replaceState = vi.spyOn(window.history, 'replaceState');
    render(<InvitationActivationScreen />);
    fireEvent.change(screen.getByLabelText(/Nueva contraseña/), {
      target: { value: 'twelve-chars-password' },
    });
  ...
  it('shows an expired invitation without exposing the provider response', async () => {
    navigation.pathname = '/activate';
    navigation.search = 'token=expired-secret';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        json(
  ...
  it('completes no-email activation without persistence or logging', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Usuario institucional'), {
      target: { value: 'sofia.herrera' },
    });
    fireEvent.change(screen.getByLabelText('Código de activación'), {
  ...
  it('always shows the generic accepted recovery state', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn(async () => json({ accepted: true }, 202)),
    );
    render(<ForgotPasswordScreen />);
    fireEvent.change(
      screen.getByLabelText('Usuario institucional o correo verificado'),
      { target: { value: 'possibly-unknown' } },
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'Solicitar recuperación' }),
    );
    expect(
      await screen.findByRole('heading', { name: 'Solicitud recibida' }),
    ).toBeTruthy();
    expect(
      screen.getByText(/Si la cuenta puede recuperar acceso/i),
    ).toBeTruthy();
  });
  ...
  it('confirms a reset token and removes it from the URL', async () => {
    ...
    const fetchMock = vi.fn<typeof fetch>(async () =>
      json({ status: 'PASSWORD_RESET', revokedSessions: 2 }),
    );
    vi.stubGlobal('fetch', fetchMock);
    const replaceState = vi.spyOn(window.history, 'replaceState');
  ...
  it('handles an expired reset token with the same safe terminal copy', async () => {
    navigation.pathname = '/reset-password';
    navigation.search = 'token=expired-reset';
    vi.stubGlobal(
      'fetch',
      vi.fn(async () =>
        json(
