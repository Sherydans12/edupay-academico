    ...
    : /Chrome\//.test(navigator.userAgent)
      ...
      : /Firefox\//.test(navigator.userAgent)
        ...
        : /Safari\//.test(navigator.userAgent)
          ? 'Safari'
          : 'Navegador';
  const system = /Windows/.test(navigator.userAgent)
    ? 'Windows'
...
function publicIdentityClient(): IdentityBrowserClient {
  return new IdentityBrowserClient({
...
function errorTitle(error: unknown): string {
  if (error instanceof IdentityApiError && error.status === 410)
    return 'El enlace ya no está disponible';
...
export function LoginScreen() {
  ...
  async function submit(
    ...
    try {
      ...
      if (
        nextError instanceof IdentityApiError &&
        nextError.code === 'MEMBERSHIP_SELECTION_REQUIRED'
      ) {
      ...
      <form className="account-form" onSubmit={submit}>
        {error ? (
          <Alert
            ...
            tone={choices.length ? 'info' : 'error'}
          >
            {error}
          </Alert>
        ) : null}
        {choices.length ? (
          <fieldset className="membership-choices">
            <legend>Institución</legend>
            {choices.map((choice) => (
              <button
                key={choice.membershipId}
                onClick={(event) => {
                  setTenantHandle(choice.tenantHandle);
                  void submit(event, choice.tenantHandle);
                }}
                type="button"
              >
                <strong>{choice.tenantHandle}</strong>
                <span>{choice.roles.join(' · ')}</span>
              </button>
            ))}
          </fieldset>
        ) : (
          <>
            ...
            <Button className="account-submit" loading={loading} type="submit">
              Entrar
            </Button>
          </>
        )}
        <div className="account-links">
          ...
          <Link href="/activate-code">Tengo un código de activación</Link>
        </div>
      </form>
    </AccountShell>
  );
...
function SecretPasswordFlow({ kind }: { kind: 'invitation' | 'reset' }) {
  ...
  return (
    <AccountShell
      title={isInvitation ? 'Activa tu cuenta' : 'Crea una nueva contraseña'}
      description={
        isInvitation
          ? 'Elige tu contraseña permanente. La invitación define la institución y el rol; no puedes cambiarlos desde este enlace.'
          : 'Este enlace es personal y de un solo uso. Al confirmar, Identity revocará las sesiones que correspondan.'
      }
    >
      {state === 'success' ? (
        ...
        <form className="account-form" onSubmit={submit}>
          {error ? (
            <Alert title={errorTitle(error)} tone="error">
          ...
          <Button
            ...
            type="submit"
          >
            {isInvitation ? 'Activar cuenta' : 'Guardar nueva contraseña'}
          </Button>
        </form>
      ) : (
        <Alert title="Falta el enlace seguro" tone="warning">
          Abre nuevamente el enlace completo que recibiste. Por seguridad, no es
          posible continuar sin su token.
        </Alert>
      )}
    </AccountShell>
  );
...
export function InvitationActivationScreen() {
  return <SecretPasswordFlow kind="invitation" />;
}
export function ResetPasswordScreen() {
  return <SecretPasswordFlow kind="reset" />;
}
...
export function ActivationCodeScreen() {
  ...
  return (
    <AccountShell
      title="Activa tu acceso sin correo"
...
export function ForgotPasswordScreen() {
  ...
  async function submit(event: FormEvent) {
    ...
    setError(null);
    try {
      await client.requestPasswordRecovery(
        identifier,
  ...
  return (
    ...
    >
      {accepted ? (
        ...
        <form className="account-form" onSubmit={submit}>
          {error ? (
            <Alert title="No pudimos recibir la solicitud" tone="error">
              {identityErrorMessage(error)}
            </Alert>
          ) : null}
          <label className="account-field">
