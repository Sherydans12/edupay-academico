...
const accents: SubjectCardViewModel['accent'][] = [
  'blue',
  'turquoise',
  'purple',
  'yellow',
...
export function deliverableItems(units: LearningUnitWithItems[]) {
  return units.flatMap((unit) =>
    unit.items.filter(
      (item) => item.type === 'ASSIGNMENT' || item.type === 'ASSESSMENT',
    ),
  );
}
...
export function errorCopy(
  error: unknown,
  fallback = 'No pudimos cargar la información. Revisa tu conexión e inténtalo nuevamente.',
) {
  if (error instanceof AcademicApiError) {
    if (error.status === 0)
      return { title: 'No pudimos conectar', body: error.message };
    if (error.status === 401)
      return { title: 'Sesión expirada', body: error.message };
    if (error.status === 403)
      return {
        title: 'Acceso no autorizado',
        body: 'El servidor no reconoce una relación activa que permita ver este contenido.',
      };
    if (error.status === 404)
      return {
        title: 'Contenido no encontrado',
        body: 'Puede que este contenido haya sido archivado o ya no esté disponible.',
      };
    if (error.status === 409)
      return { title: 'El contenido cambió', body: error.message };
...
export function isSensitiveConfirmationError(error: unknown) {
  return (
    ...
    error.status === 409 &&
    /confirm|confirmation|evidence|student work|published|scheduled/i.test(
      error.message,
    )
  );
}
