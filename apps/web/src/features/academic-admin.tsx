...
function SyncStatusOverview({ api }: { api: AcademicApiClient }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { void api.getSyncStatus().then(setStatus).catch(() => setStatus(null)); }, 0);
    return () => window.clearTimeout(timer);
  }, [api]);
  if (!status) return null;
  return <section aria-labelledby="sync-status-title" className="academic-panel"><div className="section-heading"><div><h2 id="sync-status-title">Sincronización EduPay</h2><p>{status.configured ? `${status.configuration?.sourceTenantId} · ${status.configuration?.academicYearLabel}` : 'La sincronización aún no está configurada para este tenant.'}</p></div><Badge tone={status.lastRun?.status === 'SUCCEEDED' ? 'success' : 'neutral'}>{status.lastRun?.status ?? (status.configuration?.enabled ? 'Pendiente' : 'Deshabilitada')}</Badge></div>{status.lastRun ? <p>Última ejecución: {status.lastRun.mode === 'FULL' ? 'reconciliación completa' : 'incremental'} · {status.lastRun.counts.failed + status.lastRun.counts.conflicted} conflictos o fallas.</p> : null}</section>;
}
...
function PersonList({ api, empty, identityActions, items, kind, onLinked }: { api: AcademicApiClient; empty: string; identityActions?: AccountProvisioningActions | undefined; items: AdminData['students']; kind: 'student' | 'teacher'; onLinked: () => void }) { return items.length ? <ul className="admin-list person-access-list">{items.map((item) => <li key={item.id}><span><strong>{item.firstName} {item.lastName}</strong><small>{item.email ?? 'Sin correo'} · {item.identityUserId ? 'Identidad vinculada' : 'Sin acceso de Identity'}{item.source === 'EDUPAY' ? ' · Campos administrativos gestionados por EduPay' : ''}</small></span><div className="person-access-list__actions">{item.source === 'EDUPAY' ? <Badge tone="info">EduPay</Badge> : null}<Badge tone={item.status === 'ACTIVE' ? 'success' : 'neutral'}>{statusLabel(item.status)}</Badge><AccountProvisioning api={api} identityActions={identityActions} kind={kind} onLinked={onLinked} person={item} /></div></li>)}</ul> : <EmptyState title="Sin registros" description={empty} />; }
