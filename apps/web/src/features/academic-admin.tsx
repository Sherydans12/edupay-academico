  ...
  return (
    <>
      <Alert title="Datos académicos reales" tone="success">
        ...
        membresías y sesiones siguen perteneciendo a EduPay Identity.
      </Alert>
      <div className="compact-stats academic-stats">
        ...
        <Card className="compact-stat">
          ...
          <div>
            ...
            <small>espacios de asignatura</small>
          </div>
        </Card>
        <Card className="compact-stat">
          <span>
            ...
            <div>
              ...
              <div>
                ...
                <p>
                  ...
                  cursos · {data.subjects.length} materias catalogadas
                </p>
              </div>
            </div>
        ...
        <Card>
          <div className="admin-card-heading">
            <div>
              ...
              <div>
                <h2>Personas y acceso académico</h2>
                <p>
                  {data.students.length} estudiantes · {data.teachers.length}{' '}
                  docentes con registros separados de Identity
                </p>
              </div>
            </div>
            <Badge tone="info">API</Badge>
...
function StructureView({
  ...
  const courseSubjects = data.courseSubjects.filter(
    (courseSubject) => courseSubject.courseId === selectedCourseId,
  );
  return (
      ...
      <section className="academic-panel">
        <div className="section-heading">
          <div>
            ...
            <p>Los años cerrados y archivados se conservan; no se eliminan.</p>
          </div>
        </div>
          ...
          <table>
            ...
            <tbody>
              {data.academicYears.map((year) => (
                <tr key={year.id}>
                  <td data-label="Nombre">{year.label}</td>
                  <td data-label="Periodo">
                    {year.startDate} → {year.endDate}
                  </td>
                  <td data-label="Estado">
                    <Badge
                      tone={year.status === 'ACTIVE' ? 'success' : 'neutral'}
                    >
      ...
      <section className="academic-panel">
        <div className="section-heading">
          <div>
            ...
            <p>
              Un curso pertenece a un año académico y reúne a sus estudiantes.
            </p>
          </div>
          <Select
            id="structure-course"
        ...
        {selectedCourse ? (
          ...
          <EmptyState
            ...
            description="Elige un curso para revisar su roster."
          />
        )}
      </section>
      <section className="academic-panel">
        <div className="section-heading">
          <div>
            <h2>Catálogo y configuración de CourseSubjects</h2>
            <p>
              Subject es el catálogo reutilizable; CourseSubject es el espacio
        ...
        <div className="admin-inline-grid">
          <div>
            <h3>Catálogo de subjects</h3>
            <ul className="admin-list">
              {data.subjects.map((subject) => (
                <li key={subject.id}>
                  <span>{subject.name}</span>
                  <Badge tone="neutral">{statusLabel(subject.status)}</Badge>
                </li>
              ))}
            </ul>
          </div>
          <div>
            <h3>
              {selectedCourse
                ? `CourseSubjects · ${selectedCourse.label}`
                : 'CourseSubjects'}
            </h3>
            {courseSubjects.length ? (
              <ul className="admin-list">
                {courseSubjects.map((courseSubject) => (
                  <li key={courseSubject.id}>
                    <span>
                      {courseSubject.subject?.name ??
                        data.subjects.find(
                          (subject) => subject.id === courseSubject.subjectId,
                        )?.name ??
                        courseSubject.subjectId}
                      <small>
                        {courseSubject.defaultForCourse
                          ? 'Por defecto para el curso'
                          : 'Asignación directa'}
                      </small>
                    </span>
                    <Badge
                    ...
                    >
                      {statusLabel(courseSubject.status)}
                    </Badge>
                  </li>
                ))}
              </ul>
              ...
              <EmptyState
                title="Sin CourseSubjects"
                description="Este curso aún no tiene espacios de asignatura configurados."
              />
            )}
          </div>
        </div>
...
function CourseRoster({
  ...
  if (!roster.length)
    return (
      <EmptyState
        title="Roster vacío"
        description={`Todavía no hay estudiantes inscritos en ${course.label}.`}
      />
    );
  return (
    <div className="responsive-table">
...
function PersonForm({
  ...
  return (
    <form className="academic-form" onSubmit={submit}>
      <h3>Nuevo {kind === 'student' ? 'estudiante' : 'docente'}</h3>
      <div className="academic-form__fields">
        <Input
          id={`${kind}-first-name`}
          label="Nombres"
    ...
    <div className="academic-stack">
      <div className="admin-two-column">
        <section className="academic-panel">
          ...
          <PersonList
            api={api}
            empty="Aún no hay estudiantes registrados."
        ...
        <section className="academic-panel">
          ...
          <PersonList
            api={api}
            empty="Aún no hay docentes registrados."
            identityActions={identityActions}
            items={data.teachers}
            kind="teacher"
            onLinked={onSaved}
      ...
      <section className="academic-panel">
        <div className="section-heading">
          <div>
            <h2>Inscripciones y asignaciones</h2>
            <p>
              Estas relaciones se guardan en Académico; Identity solo aporta la
              identidad confiable.
            </p>
          </div>
        </div>
        ...
        <div className="assignment-form-grid">
          <form
            ...
            onSubmit={(event) => {
              event.preventDefault();
              void run(() => api.enrollStudent({ studentId, courseId }));
            }}
          >
            <h3>Inscribir en curso</h3>
            <Select
              id="enroll-student"
              label="Estudiante"
              value={studentId}
          ...
          >
            <h3>Asignar docente a CourseSubject</h3>
            <Select
            ...
            <Button type="submit" disabled={!courseSubjectId || !teacherId}>
              Guardar asignación
            </Button>
          </form>
          <form
          ...
          >
            <h3>Asignar subject directamente</h3>
            <Select
              id="direct-student"
              label="Estudiante"
              value={studentId}
            ...
            <Button type="submit" disabled={!studentId || !courseSubjectId}>
              Asignar subject
            </Button>
          </form>
        </div>
...
function PersonList({
  ...
  empty: string;
  identityActions?: AccountProvisioningActions | undefined;
  items: AdminData['students'];
  kind: 'student' | 'teacher';
  onLinked: () => void;
}) {
  return items.length ? (
    <ul className="admin-list person-access-list">
      {items.map((item) => (
        <li key={item.id}>
          <span>
            <strong>
            ...
            <small>
              ...
              {item.identityUserId
                ? 'Identidad vinculada'
                : 'Sin acceso de Identity'}
              {item.source === 'EDUPAY'
                ? ' · Campos administrativos gestionados por EduPay'
                : ''}
            </small>
          </span>
          <div className="person-access-list__actions">
            {item.source === 'EDUPAY' ? (
    ...
    <EmptyState title="Sin registros" description={empty} />
  );
}
      ...
      <PageHeading
        description={
          view === 'overview'
            ...
            : view === 'structure'
              ? 'Configura años, cursos, roster y CourseSubjects sin salir del espacio académico.'
              : 'Administra registros académicos y crea accesos de Identity desde cada persona.'
        }
        title={
          view === 'overview'
