...
function PeopleView({ api, data, onSaved }: { api: AcademicApiClient; data: AdminData; onSaved: () => void }) {
  ...
  return <div className="academic-stack"><div className="admin-two-column"><section className="academic-panel"><h2>Estudiantes</h2><PersonForm api={api} kind="student" onSaved={onSaved} /><PersonList empty="Aún no hay estudiantes registrados." items={data.students.map((student) => ({ id: student.id, name: `${student.firstName} ${student.lastName}`, detail: student.email ?? 'Sin correo', status: student.status }))} /></section><section className="academic-panel"><h2>Docentes</h2><PersonForm api={api} kind="teacher" onSaved={onSaved} /><PersonList empty="Aún no hay docentes registrados." items={data.teachers.map((teacher) => ({ id: teacher.id, name: `${teacher.firstName} ${teacher.lastName}`, detail: teacher.email ?? 'Sin correo', status: teacher.status }))} /></section></div><section className="academic-panel"><div className="section-heading"><div><h2>Inscripciones y asignaciones</h2><p>Estas relaciones se guardan en Académico; Identity solo aporta la identidad confiable.</p></div></div>{actionError ? <Alert title="No se pudo guardar la relación" tone="error">{actionError}</Alert> : null}<div className="assignment-form-grid"><form className="academic-form" onSubmit={(event) => { event.preventDefault(); void run(() => api.enrollStudent({ studentId, courseId })); }}><h3>Inscribir en curso</h3><Select id="enroll-student" label="Estudiante" value={studentId} onChange={(event) => setStudentId(event.target.value)}>{data.students.map((student) => <option key={student.id} value={student.id}>{student.firstName} {student.lastName}</option>)}</Select><Select id="enroll-course" label="Curso" value={courseId} onChange={(event) => setCourseId(event.target.value)}>{data.courses.map((course) => <option key={course.id} value={course.id}>{course.label}</option>)}</Select><Button type="submit" disabled={!studentId || !courseId}>Inscribir estudiante</Button></form><form className="academic-form" onSubmit={(event) => { event.preventDefault(); void run(() => api.assignCourseSubjectTeachers({ courseSubjectId, teacherIds: [teacherId] })); }}><h3>Asignar docente a CourseSubject</h3><Select id="assignment-course-subject" label="CourseSubject" value={courseSubjectId} onChange={(event) => setCourseSubjectId(event.target.value)}>{availableCourseSubjects.map((item) => <option key={item.id} value={item.id}>{item.subject?.name ?? item.subjectId}</option>)}</Select><Select id="assignment-teacher" label="Docente" value={teacherId} onChange={(event) => setTeacherId(event.target.value)}>{data.teachers.map((teacher) => <option key={teacher.id} value={teacher.id}>{teacher.firstName} {teacher.lastName}</option>)}</Select><Button type="submit" disabled={!courseSubjectId || !teacherId}>Guardar asignación</Button></form><form className="academic-form" onSubmit={(event) => { event.preventDefault(); void run(() => api.directlyEnrollStudent({ studentId, courseSubjectId })); }}><h3>Asignar subject directamente</h3><Select id="direct-student" label="Estudiante" value={studentId} onChange={(event) => setStudentId(event.target.value)}>{data.students.map((student) => <option key={student.id} value={student.id}>{student.firstName} {student.lastName}</option>)}</Select><Select id="direct-course-subject" label="CourseSubject" value={courseSubjectId} onChange={(event) => setCourseSubjectId(event.target.value)}>{availableCourseSubjects.map((item) => <option key={item.id} value={item.id}>{item.subject?.name ?? item.subjectId}</option>)}</Select><Button type="submit" disabled={!studentId || !courseSubjectId}>Asignar subject</Button></form></div></section></div>;
}

function PersonList({ empty, items }: { empty: string; items: Array<{ id: string; name: string; detail: string; status: string }> }) { return items.length ? <ul className="admin-list">{items.map((item) => <li key={item.id}><span><strong>{item.name}</strong><small>{item.detail}</small></span><Badge tone={item.status === 'ACTIVE' ? 'success' : 'neutral'}>{statusLabel(item.status)}</Badge></li>)}</ul> : <EmptyState title="Sin registros" description={empty} />; }
