...
function adminClient(
  ...
  return {
    ...
    listCourseSubjects: vi.fn(async () => ({
      items: [courseSubject1],
      nextCursor: null,
    })),
    ...
    createSubject: vi.fn(async (input) => ({
      id: 'new-sub',
      name: input.name,
      status: 'ACTIVE' as const,
      createdAt: timestamp,
    ...
    updateSubject: vi.fn(async (subId, input) => ({
      id: subId,
    ...
    createCourseSubject: vi.fn(async (input) => ({
      id: 'new-cs',
      courseId: input.courseId,
      subjectId: input.subjectId,
      defaultForCourse: input.defaultForCourse ?? true,
      sortOrder: input.sortOrder ?? 0,
      status: 'ACTIVE' as const,
      createdAt: timestamp,
      updatedAt: timestamp,
    })),
    updateCourseSubject: vi.fn(async (csId, input) => ({
      id: csId,
      courseId: id,
      subjectId: id,
      defaultForCourse: input.defaultForCourse ?? true,
    ...
    createTeacher: vi.fn(async (input) => ({
      id: 'new-teach',
      identityUserId: null,
      source: 'MANUAL',
      externalReference: null,
    ...
    updateTeacher: vi.fn(async (teachId, input) => ({
      id: teachId,
      identityUserId: null,
      source: 'MANUAL',
      externalReference: null,
      firstName: input.firstName ?? 'Teach',
      lastName: input.lastName ?? 'Teach',
      email: input.email ?? null,
      status: 'ACTIVE' as const,
      createdAt: timestamp,
      updatedAt: timestamp,
    ...
    updateStudent: vi.fn(async (stuId, input) => ({
      id: stuId,
      identityUserId: null,
      source: 'MANUAL',
      externalReference: null,
      firstName: input.firstName ?? 'Student',
      lastName: input.lastName ?? 'Student',
      email: input.email ?? null,
      status: 'ACTIVE' as const,
      createdAt: timestamp,
      updatedAt: timestamp,
    ...
    directlyEnrollStudent: vi.fn(async () => ({
      id: 'dir-1',
      courseSubjectId: id,
      studentId: id,
      status: 'ACTIVE' as const,
...
describe('Academic admin screens', () => {
  ...
  it('renders structure view with years, courses and switches between tabs', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Catálogo de asignaturas' }),
    ).toBeTruthy();
    expect(
  ...
  it('creates and archives a subject from the Subject Catalog tab', async () => {
    ...
    render(<AcademicAdminScreen api={client} view="structure" />);
    fireEvent.click(
      await screen.findByRole('tab', { name: 'Catálogo de Asignaturas' }),
    );
    ...
    await waitFor(() => {
      expect(client.createSubject).toHaveBeenCalledWith({
    ...
    await waitFor(() => {
      expect(client.updateSubject).toHaveBeenCalledWith(id, {
        status: 'ARCHIVED',
      });
    });
  });
  ...
  it('manages course subjects and teacher assignments in structure view', async () => {
    ...
    const removeBtn = screen.getByRole('button', {
      name: /desasignar a gabriela/i,
    });
    fireEvent.click(removeBtn);
    await waitFor(() => {
      expect(client.deactivateTeacherAssignment).toHaveBeenCalledWith(
        'assign-1',
      );
    });
  });
  ...
  it('prevents modifying name fields on EduPay-sourced student records', async () => {
    ...
    expect(
      screen.getByText(
        /el nombre y apellido provienen de la sincronización con edupay/i,
      ),
    ).toBeTruthy();
  ...
  it('locks email editing for already linked accounts to prevent divergence from Identity', async () => {
    ...
    expect(dialog).toBeTruthy();
    expect(
      screen.getByText(/esta cuenta ya tiene acceso de identity vinculado/i),
    ).toBeTruthy();
  ...
  it('renders teacher management and allows searching and editing', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Profesores' }),
    ).toBeTruthy();
  ...
  it('renders enrollments and assignments tab with direct assignment workflows', async () => {
    ...
    expect(
      await screen.findByRole('heading', {
        name: 'Inscripciones y asignaciones directas',
      }),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Inscribir alumno' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Guardar asignación' }),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Asignar directamente' }),
    ).toBeTruthy();
  });
  ...
  it('renders a forbidden state without pretending data is available', async () => {
    ...
    expect(
      await screen.findByText('Sin permiso para este espacio'),
    ).toBeTruthy();
