...
const subject = { id: subjectId, courseId: '00000000-0000-4000-8000-000000000003', subjectId: '00000000-0000-4000-8000-000000000004', defaultForCourse: true, sortOrder: 0, status: 'ACTIVE' as const, course: { id: '00000000-0000-4000-8000-000000000003', academicYearId: '00000000-0000-4000-8000-000000000005', label: '7º Básico A', status: 'ACTIVE' as const, createdAt: timestamp, updatedAt: timestamp }, subject: { id: '00000000-0000-4000-8000-000000000004', name: 'Lenguaje y Comunicación', status: 'ACTIVE' as const, createdAt: timestamp, updatedAt: timestamp }, createdAt: timestamp, updatedAt: timestamp };
...
function item(id: string, title: string, publicationStatus: 'DRAFT' | 'SCHEDULED' | 'PUBLISHED' | 'ARCHIVED', type: 'MATERIAL' | 'ASSIGNMENT' = 'MATERIAL') {
  return { id, courseSubjectId: subjectId, learningUnitId: unitId, type, title, description: `${title} descripción`, content: type === 'MATERIAL' ? 'Contenido real.' : null, instructions: type === 'ASSIGNMENT' ? 'Instrucciones reales.' : null, body: null, sortOrder: 0, publicationStatus, publishAt: publicationStatus === 'SCHEDULED' ? '2099-08-12T20:00:00+00:00' : null, publishedAt: publicationStatus === 'PUBLISHED' ? timestamp : null, publishedByIdentityUserId: publicationStatus === 'PUBLISHED' ? 'teacher-1' : null, dueAt: type === 'ASSIGNMENT' ? '2026-08-12T20:00:00+00:00' : null, createdByIdentityUserId: 'teacher-1', updatedByIdentityUserId: null, createdAt: timestamp, updatedAt: timestamp };
}
...
const route = { courseSubjectId: subjectId, units: [{ id: unitId, courseSubjectId: subjectId, title: 'Unidad real', description: 'Descripción de unidad', sortOrder: 0, startAt: null, endAt: null, status: 'ACTIVE' as const, createdAt: timestamp, updatedAt: timestamp, items: [visibleMaterial, futureItem, draftItem, assignment] }] };
