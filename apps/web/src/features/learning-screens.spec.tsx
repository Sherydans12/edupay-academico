...
function item(
  ...
  return {
    ...
    content: type === 'MATERIAL' ? 'Contenido real.' : null,
    instructions: type === 'ASSIGNMENT' ? 'Instrucciones reales.' : null,
    body: null,
    sortOrder: 0,
    publicationStatus,
    publishAt:
      publicationStatus === 'SCHEDULED' ? '2099-08-12T20:00:00+00:00' : null,
    publishedAt: publicationStatus === 'PUBLISHED' ? timestamp : null,
    publishedByIdentityUserId:
      publicationStatus === 'PUBLISHED' ? 'teacher-1' : null,
    dueAt: type === 'ASSIGNMENT' ? '2026-08-12T20:00:00+00:00' : null,
  ...
  units: [
    {
      ...
      createdAt: timestamp,
      updatedAt: timestamp,
      items: [visibleMaterial, futureItem, draftItem, assignment],
    },
  ],
};
...
describe('real Learning screens', () => {
  it('renders the student hierarchy and hides draft and future scheduled items defensively', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'Unidad real' }),
    ).toBeTruthy();
  ...
  it('renders actual item detail with the connected submission workflow', async () => {
    render(
      <StudentAssignmentScreen
        api={api({
          getStudentContextSubjects: vi.fn(async () => [subject]),
          getLearningItem: vi.fn(async () => assignment),
        })}
        courseSubjectId={subjectId}
        learningItemId={assignment.id}
      />,
    );

    expect(
      await screen.findByRole('heading', { name: 'Actividad real' }),
    ).toBeTruthy();
  ...
  it('supports typed authoring, publication actions, and accessible item reorder', async () => {
    ...
    const teacherRoute = {
      ...route,
      units: [{ ...route.units[0]!, items: [visibleMaterial, second] }],
    };
    const reorderLearningItems = vi.fn(
      async () => teacherRoute.units[0]!.items,
    );
    const createLearningItem = vi.fn(async () => assignment);
    const client = api({
      getTeacherContextSubjects: vi.fn(async () => [subject]),
      getLearningRoute: vi.fn(async () => teacherRoute),
    ...
    fireEvent.click(
      screen.getByRole('button', {
        name: /mover material visible hacia abajo/i,
      }),
    );
    expect(reorderLearningItems).toHaveBeenCalledWith(unitId, {
      orderedIds: [second.id, visibleMaterial.id],
    });
    await waitFor(() =>
      expect(client.getLearningRoute).toHaveBeenCalledTimes(2),
    );

    fireEvent.click(
      screen.getByRole('button', { name: /agregar contenido a unidad real/i }),
    );
    ...
    fireEvent.change(screen.getByLabelText('Instrucciones'), {
      target: { value: 'Entrega el documento.' },
    });
    ...
    await waitFor(() =>
      expect(createLearningItem).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          dueAt: '2026-08-20T22:00:00.000Z',
        }),
      ),
    );
  });
  ...
  it('creates units and renders publication states with schedule and archive actions', async () => {
    ...
    const publicationRoute = {
      ...route,
      units: [{ ...route.units[0]!, items: [draftItem, futureItem] }],
    };
    const client = api({
      getTeacherContextSubjects: vi.fn(async () => [subject]),
    ...
    fireEvent.change(screen.getByLabelText('Título'), {
      target: { value: 'Unidad nueva' },
    });
    fireEvent.click(screen.getByRole('button', { name: 'Crear unidad' }));
    await waitFor(() =>
      expect(createLearningUnit).toHaveBeenCalledWith(
        expect.objectContaining({
          courseSubjectId: subjectId,
          title: 'Unidad nueva',
          sortOrder: 0,
    ...
    await waitFor(() =>
      expect(publishLearningItem).toHaveBeenCalledWith(draftItem.id),
    );

    fireEvent.click(screen.getByRole('button', { name: 'Programar' }));
    fireEvent.change(screen.getByLabelText('Programar publicación'), {
      target: { value: '2099-08-20T18:00' },
    });
    fireEvent.click(
      screen.getByRole('button', { name: 'Guardar programación' }),
    );
    await waitFor(() =>
      expect(scheduleLearningItem).toHaveBeenCalledWith(
        draftItem.id,
        expect.objectContaining({
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Archivar Borrador privado' }),
    );
    await waitFor(() =>
      expect(archiveLearningItem).toHaveBeenCalledWith(draftItem.id),
    );
  });
    ...
    const updateLearningItem = vi
      ...
      .mockRejectedValueOnce(
        new AcademicApiError({
          ...
          details: [],
          message: 'Published content changes require explicit confirmation.',
