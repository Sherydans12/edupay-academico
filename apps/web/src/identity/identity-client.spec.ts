...
describe('IdentityBrowserClient', () => {
  it('uses the browser cookie transport for login, refresh, logout, and logout-all without exposing a refresh token', async () => {
    ...
    await expect(
      client.login({
        identifier: 'sofia',
        password: 'secret',
        device: { label: 'Chrome en Windows' },
      }),
    ).resolves.toEqual(token);
    await expect(client.refresh()).resolves.toEqual(token);
    await expect(client.logout('access-token')).resolves.toBeUndefined();
    await expect(client.logoutAll('access-token')).resolves.toEqual({
      revokedSessions: 3,
    });

    for (const index of [0, 1, 2, 3]) {
      expect(fetchImpl.mock.calls[index]?.[1]).toMatchObject({
        credentials: 'include',
      });
    }
  ...
  it('matches current profile, membership, and replacement-token context contracts', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        json({
          ...
          session: {
            id: 'session-1',
            authenticatedAt: '2026-08-09T12:00:00Z',
            activeMembership: membership,
          },
        }),
      )
    ...
    expect(fetchImpl).toHaveBeenNthCalledWith(
      1,
      'https://identity.example.test/api/v1/auth/me',
      expect.objectContaining({ method: 'GET' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      'https://identity.example.test/api/v1/auth/memberships',
      expect.objectContaining({ method: 'GET' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      3,
      'https://identity.example.test/api/v1/auth/sessions/current-context',
  ...
  it('sends exact invitation, activation, and recovery request fields', async () => {
    ...
    await client.completeActivation(
      'sofia.herrera',
      'act-secret',
      'chosen-password',
    );
  ...
  it('fixes the provisioned role and supports both activation delivery methods without cookie credentials', async () => {
    const provisioned = {
      ...
      roles: ['STUDENT'],
      activation: {
        emailInvitationAvailable: true,
        activationChallengeAvailable: false,
      },
    };
    const fetchImpl = vi
      .fn()
    ...
    await client.provisionMembership('access-token', 'tenant-1', {
      institutionalUsername: 'sofia.herrera',
      roles: ['STUDENT'],
    });
    await client.inviteMembership('access-token', 'tenant-1', 'membership-1');
    await client.createActivationChallenge(
      'access-token',
      'tenant-1',
      'membership-1',
    );
    ...
    expect(
      fetchImpl.mock.calls.every((call) => call[1]?.credentials !== 'include'),
    ).toBe(true);
  });
  ...
  it('preserves stable safe error metadata without leaking an unknown response body', async () => {
    ...
    await expect(
      client.acceptInvitation('expired', 'chosen-password'),
