...
const identityRoleSchema = z.enum([
  'SYSTEM_ADMIN',
  'TENANT_ADMIN',
  'TEACHER',
  'STUDENT',
...
export const provisionedMembershipSchema = z
  .object({
    userId: z.string().min(1),
    membershipId: z.string().min(1),
    tenantId: z.string().min(1),
...
export const invitationStateSchema = z
  .object({
    membershipId: z.string().min(1),
...
export const activationChallengeSchema = z
  .object({
    membershipId: z.string().min(1),
...
export function identityErrorMessage(error: unknown): string {
  ...
  if (error.status === 401)
    return 'No pudimos verificar las credenciales. Revisa los datos e inténtalo nuevamente.';
  if (error.status === 403)
    return 'Tu cuenta no tiene permiso para completar esta acción.';
  if (error.status === 404) return 'El recurso solicitado no está disponible.';
  if (error.status === 409)
    return 'La cuenta o membresía requiere resolver un conflicto antes de continuar.';
  if (error.status === 410)
    return 'Este enlace o código expiró o ya fue utilizado. Solicita uno nuevo.';
  if (error.status === 429)
...
export class IdentityBrowserClient {
  ...
  logoutAll(accessToken: string) {
    return this.request(
      ...
      z.object({ revokedSessions: z.number().int().nonnegative() }).strict(),
      {
        method: 'POST',
        accessToken,
        browserCredentials: true,
  ...
  me(accessToken: string) {
    return this.request('/api/v1/auth/me', identityMeSchema, {
      method: 'GET',
      accessToken,
    });
  }
  ...
  memberships(accessToken: string) {
    return this.request(
      ...
      z.array(identityMembershipSchema),
  ...
  switchContext(accessToken: string, membershipId: string) {
    return this.request(
      '/api/v1/auth/sessions/current-context',
  ...
  completeActivation(
    ...
    return this.request(
      '/api/v1/auth/activations/complete',
      lifecycleResultSchema,
      {
        method: 'POST',
        body: JSON.stringify({
  ...
  provisionMembership(
    ...
    return this.request(
      `/api/v1/tenants/${encodeURIComponent(tenantId)}/memberships`,
      provisionedMembershipSchema,
      {
        method: 'POST',
        accessToken,
        body: JSON.stringify(input),
  ...
  inviteMembership(
    ...
    return this.request(
      `/api/v1/tenants/${encodeURIComponent(tenantId)}/memberships/${encodeURIComponent(membershipId)}/invite`,
  ...
  createActivationChallenge(
    ...
    return this.request(
      `/api/v1/tenants/${encodeURIComponent(tenantId)}/memberships/${encodeURIComponent(membershipId)}/activation-challenge`,
      activationChallengeSchema,
      {
        method: 'POST',
        accessToken,
      },
