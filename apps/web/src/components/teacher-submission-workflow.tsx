    ...
    >
      ...
      {loading ? (
          ...
          {submissions.map((submission) => {
            ...
            return (
              ...
              >
                ...
                <span>
                  <strong>{studentName(submission.studentId, roster)}</strong>
                  <small>
        ...
        <EmptyState
          ...
          description="Las entregas de estudiantes aparecerán aquí cuando sean enviadas."
        />
      )}
    </section>
  );
...
export function TeacherSubmissionDetail({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const nextRoster = await getTeacherRoster(
        api,
        nextItem.courseSubjectId,
      ).catch(() => []);
      ...
      <div className="review-layout">
        <section className="review-history-panel">
          <div className="section-heading">
            <div>
              ...
              <p>
                {item.title} · {studentName(submission.studentId, roster)}
              </p>
            </div>
          ...
          <div className="revision-list">
            {submission.revisions.map((revision) => (
              <article
                className={`revision-card${revision.id === selectedRevision?.id ? ' revision-card--selected' : ''}${revision.isLate ? ' revision-card--late' : ''}`}
                key={revision.id}
              >
                <header>
                  <button
                ...
                {revision.reviews.length ? (
                  <div className="revision-reviews">
                    <strong>Historial de revisión</strong>
                    {revision.reviews.map((reviewEntry) => (
                      <div className="review-entry" key={reviewEntry.id}>
                        ...
                        <small>{formatDate(reviewEntry.createdAt)}</small>
                      </div>
                    ))}
                  </div>
                ) : null}
        ...
        <aside className="review-panel">
          <div className="review-student">
            <span className="subject-hero__mark">
              <Icon name="people" />
            </span>
            <div>
              <h1>{studentName(submission.studentId, roster)}</h1>
              <p>
          ...
          <Textarea
            ...
            placeholder="Escribe una orientación concreta…"
            value={comment}
          />
