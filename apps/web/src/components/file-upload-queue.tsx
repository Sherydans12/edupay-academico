...
export type UploadRowStatus =
  'waiting' | 'uploading' | 'validating' | 'completed' | 'failed';
...
function normalizedExtensions(policy?: StoragePolicy | null): string[] {
  return (
    policy?.allowedExtensions?.length
...
export function uploadErrorMessage(error: unknown): string {
  if (error instanceof AcademicApiError) {
    if (
      error.code === 'UPLOAD_INTENT_EXPIRED' ||
      error.status === 410 ||
      /expired|expir/i.test(error.message)
    )
      return 'La autorización de carga expiró. Reintenta este archivo para crear una nueva autorización.';
    ...
    if (
      ...
      return 'El tipo o el contenido del archivo no coincide con los formatos permitidos.';
    if (
      error.code === 'TENANT_STORAGE_QUOTA_EXCEEDED' ||
      error.code === 'GLOBAL_STORAGE_QUOTA_EXCEEDED' ||
      /quota|cuota|storage/i.test(error.message)
    )
      return 'El almacenamiento está lleno. No se pueden subir archivos nuevos; las descargas existentes siguen disponibles.';
    if (error.status === 401)
      return 'Tu sesión expiró. Vuelve a iniciar sesión e inténtalo nuevamente.';
    if (error.status === 403)
      return 'No tienes autorización para cargar este archivo en esta actividad.';
    if (error.status === 404)
      return 'La actividad ya no está disponible para esta carga.';
    if (error.status === 409)
      return 'La carga entró en conflicto con otro cambio. Revisa el estado e inténtalo nuevamente.';
    if (error.status === 400)
      return 'Revisa el nombre, formato y tamaño del archivo e inténtalo nuevamente.';
    if (error.status === 429)
      return 'Hay demasiadas cargas en curso. Espera un momento y reintenta este archivo.';
    if (error.status === 0 || error.code === 'NETWORK_ERROR')
      return 'No pudimos conectar con Académico. Conservamos el archivo para que puedas reintentar.';
    if (error.status >= 500)
      return 'El servicio de archivos no está disponible temporalmente. Reintenta este archivo.';
...
export function submissionErrorMessage(error: unknown): string {
  if (error instanceof AcademicApiError) {
    if (error.status === 401)
      return 'Tu sesión expiró. Vuelve a iniciar sesión e inténtalo nuevamente.';
    if (error.status === 403)
      return 'No tienes autorización para enviar trabajo en esta actividad.';
    if (error.status === 404)
      return 'La actividad o la entrega ya no está disponible.';
    if (error.status === 409)
      return 'La entrega cambió mientras la enviabas. Actualiza el estado e inténtalo nuevamente.';
    if (error.status === 400)
      return 'Revisa que haya entre 1 y 20 archivos finalizados e inténtalo nuevamente.';
    if (error.status >= 500)
      return 'No pudimos registrar la entrega temporalmente. Tus archivos finalizados siguen disponibles para reintentar.';
  }
...
function localFileError(
  ...
  if (file.size > maxSize)
    return `Supera el máximo de ${formatFileSize(maxSize)} por archivo.`;
  if (!normalizedExtensions(policy).includes(extensionOf(file.name)))
    return 'Este formato no está permitido para archivos académicos.';
  return undefined;
}
...
export function useFileUploadQueue({
  ...
  const addFiles = useCallback(
    (files: FileList | File[]) => {
      ...
      setRows((current) => {
        ...
        if (remaining <= 0) {
          setSelectionError(
            `Puedes adjuntar hasta ${maxFiles} archivos por entrega.`,
          );
          return current;
        }
        if (selected.length > remaining)
          setSelectionError(
            `Solo se agregaron ${remaining} archivos. El máximo por entrega es ${maxFiles}.`,
          );
        const known = new Set(
          current.map((row) => `${row.filename}-${row.sizeBytes}`),
        );
        ...
        if (!additions.length && selected.length)
          setSelectionError('Esos archivos ya están en la lista.');
  ...
  const uploadOne = useCallback(
    async (id: string): Promise<boolean> => {
      ...
      if (!row?.file || row.status === 'completed')
  ...
  const uploadPending = useCallback(async () => {
    const pendingIds = rowsRef.current
      .filter((row) => row.status === 'waiting' && row.file)
      .map((row) => row.id);
    let cursor = 0;
    const worker = async () => {
    ...
    return pendingIds.every(
      (id) =>
        rowsRef.current.find((row) => row.id === id)?.status === 'completed',
    );
  }, [uploadOne]);
  ...
  return useMemo(
    () => ({
      ...
      addFiles,
      clear,
      completedFileIds: rows.flatMap((row) =>
        row.result?.id ? [row.result.id] : [],
      ),
      getCompletedFileIds,
...
function statusLabel(row: UploadRow): string {
  ...
  if (row.status === 'uploading')
    return `Subiendo${row.progress ? ` · ${row.progress}%` : ''}`;
  ...
  return (
    <>
      <input
        ...
        onChange={(event) => {
          if (event.currentTarget.files)
            queue.addFiles(event.currentTarget.files);
      ...
      <label
        ...
        className={`upload-dropzone${disabled || busy ? ' upload-dropzone--disabled' : ''}`}
        htmlFor={id}
      >
        <span>
          <Icon name="upload" />
      ...
      {queue.selectionError ? (
        <Alert title="No se agregaron todos los archivos" tone="warning">
          {queue.selectionError}
        </Alert>
      ) : null}
      ...
      {queue.rows.length ? (
        <div aria-label="Archivos seleccionados" className="selected-files">
          {queue.rows.map((row) => (
            <div
              className={`selected-file selected-file--${row.status}`}
              key={row.id}
            >
              <Icon
                name={
              ...
              <span>
                ...
                <small>
                  {formatFileSize(row.sizeBytes)} · {statusLabel(row)}
                </small>
                {row.status === 'uploading' || row.status === 'validating' ? (
                  <progress
                    aria-label={`Progreso de ${row.filename}`}
                    max="100"
                    value={row.progress}
                  />
                ) : null}
              ...
              {row.status === 'failed' && row.retryable ? (
                <Button
                  aria-label={`Reintentar ${row.filename}`}
                  loading={false}
                  onClick={() => void queue.retryFile(row.id)}
                  size="sm"
                  variant="secondary"
                >
                  Reintentar
                ...
                <Button
                  aria-label={`Quitar ${row.filename}`}
                  onClick={() => queue.removeFile(row.id)}
                  size="icon"
                  title="Quitar"
                  variant="ghost"
                >
      ...
      {action ? (
        <Button
          disabled={Boolean(disabled || busy || actionDisabled)}
