...
function categoryFor(
  item: LearningItem,
): 'LEARNING_MATERIAL' | 'ASSIGNMENT_SOURCE' | 'ASSESSMENT_SOURCE' | null {
...
export function TeacherAttachmentManager({
    ...
    try {
      ...
      setError(
        nextError instanceof AcademicApiError
          ? nextError.message
          : 'No pudimos cargar los archivos de este contenido.',
      );
    } finally {
      setLoading(false);
    }
  ...
  async function upload() {
    ...
    try {
      if (!(await queue.uploadPending()) || queue.hasFailed) {
        setUploadError(
          'Revisa los archivos con error y reintenta individualmente.',
        );
        return;
      }
      await load();
  ...
  async function detach(file: StorageFile) {
    ...
    try {
      ...
      setError(
        nextError instanceof AcademicApiError
          ? nextError.message
          : 'No pudimos quitar el archivo del contenido.',
      );
    } finally {
      setDetaching(false);
    }
  ...
  return (
    ...
    >
        ...
        <div className="attachment-storage-info">
          <div className="storage-meter-row">
            <span className="storage-meter-label">
              <Icon name="layers" />
              <span>
                {quotaLabel(usage.state)}: {formatFileSize(usage.usedBytes)}{' '}
                usados de {formatFileSize(usage.quotaBytes)} (
      ...
      {uploadError ? (
        <Alert title="No se pudieron subir todos los archivos" tone="error">
          {uploadError}
        </Alert>
      ) : null}
      ...
      {fileToDetach ? (
        ...
        >
          <div className="detach-dialog-content">
            <p>
              ¿Deseas quitar <strong>{fileToDetach.originalFilename}</strong> (
              {formatFileSize(fileToDetach.sizeBytes)})?
            </p>
            <p className="integration-note">
              <Icon name="layers" />
              Los estudiantes ya no podrán ver ni descargar este archivo
              adjunto.
            </p>
            <div className="showcase-dialog-actions">
              <Button
                disabled={detaching}
                onClick={() => setFileToDetach(null)}
                type="button"
                variant="secondary"
              >
                Cancelar
              ...
              <Button
                loading={detaching}
                onClick={() => void detach(fileToDetach)}
                type="button"
                variant="danger"
              >
                Quitar archivo
