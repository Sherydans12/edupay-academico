...
describe('shared UI foundation', () => {
  it('applies a tenant theme without tenant-specific component coupling', () => {
    ...
    expect(container.firstElementChild?.getAttribute('data-tenant-theme')).toBe(
      'colegio-conquistadores',
    );
    expect(
      screen.getByRole('button', { name: 'Continuar' }).className,
    ).not.toContain('conquistadores');
  });

  it('associates visible field labels, hints, and invalid state', () => {
    render(
      <Input
        error="Escribe un título para continuar."
        id="title"
        label="Título"
      />,
    );
  ...
  it('changes tabs with an accessible selected state', () => {
    render(
      <Tabs
        label="Contenido"
        items={[
          { id: 'route', label: 'Ruta', content: 'Ruta activa' },
          { id: 'work', label: 'Entregas', content: 'Entregas activas' },
        ]}
      />,
    );
  ...
  it('supports arrow-key navigation across the tab set', () => {
    render(
      <Tabs
        label="Contenido"
        items={[
          { id: 'route-keyboard', label: 'Ruta', content: 'Ruta activa' },
          {
            id: 'work-keyboard',
            label: 'Entregas',
