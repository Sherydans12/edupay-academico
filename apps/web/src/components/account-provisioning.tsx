...
export type AccountProvisioningActions = Pick<
  IdentitySessionContextValue,
  'provisionMembership' | 'inviteMembership' | 'createActivationChallenge'
>;
...
function academicError(error: unknown): string {
  if (error instanceof AcademicApiError) {
    if (error.status === 403)
      return 'Tu sesión no puede vincular esta persona en el tenant activo.';
    if (error.status === 404)
      return 'El registro académico ya no está disponible.';
...
export function AccountProvisioning({
  ...
  const [phase, setPhase] = useState<
    'form' | 'provisioning' | 'linking' | 'partial' | 'activation' | 'complete'
  >('form');
  ...
  async function activate(created: ProvisionedMembership) {
    ...
    try {
      if (created.activation.emailInvitationAvailable) {
        ...
        setChallenge(
          await identity.createActivationChallenge(created.membershipId),
        );
      }
      setPhase('complete');
      onLinked();
        ...
        <div className="provisioning-dialog">
          ...
          {phase === 'partial' && provisioned ? (
            <div className="provisioning-partial">
              ...
              <p>
                No se eliminó la cuenta de Identity. Estos identificadores se
                conservarán solo mientras este diálogo permanezca abierto.
              </p>
              <div className="provisioning-actions">
                <Button onClick={closeDialog} variant="secondary">
                  Cerrar
          ...
          {phase === 'complete' && invitation ? (
            <div className="provisioning-complete">
              <Alert title="Invitación solicitada" tone="success">
                Identity registró la entrega del correo. No se expuso ningún
                token de invitación.
              </Alert>
          ...
          {phase === 'complete' && challenge ? (
            <div className="provisioning-complete">
              <Alert title="Código de activación creado" tone="warning">
                Muéstralo solo a la persona indicada mediante un canal
                institucional seguro. No podrá recuperarse después de cerrar
                este diálogo.
              </Alert>
              ...
              <p>
                Vence el {expiry}. Para usarlo: abre{' '}
                <strong>/activate-code</strong>, ingresa el usuario, este código
                y una contraseña elegida por la persona.
              </p>
              <div className="provisioning-actions">
                <Button
                  onClick={() =>
                ...
                <Button onClick={closeDialog}>
                  Ya lo entregué de forma segura
                </Button>
              </div>
            </div>
          ) : null}
          {phase === 'complete' && !invitation && !challenge && provisioned ? (
            <div className="provisioning-complete">
              <p>
                El vínculo académico está confirmado. Reintenta generar el
                método de activación sin volver a crear la membresía.
              </p>
              <div className="provisioning-actions">
                <Button onClick={closeDialog} variant="secondary">
                  Cerrar
