...
function provisioned(role: 'STUDENT' | 'TEACHER', email: boolean) {
  return {
    ...
    institutionalUsername:
      role === 'STUDENT' ? 'sofia.herrera' : 'camila.rojas',
    ...(email ? { email: 'sofia@example.test' } : {}),
    status: 'PENDING_ACTIVATION' as const,
    roles: [role],
    activation: {
      emailInvitationAvailable: email,
...
function identityActions(
  ...
  return {
    provisionMembership: vi.fn(async (input) =>
      provisioned(input.role, Boolean(input.email)),
    ),
...
describe('AccountProvisioning', () => {
  it('creates only a STUDENT membership, links the returned user, and asks Identity to send the invitation', async () => {
    ...
    expect(
      screen.getByLabelText('Correo para invitación (opcional)'),
    ).toHaveProperty('value', 'sofia@example.test');
