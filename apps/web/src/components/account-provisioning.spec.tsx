...
function provisioned(role: 'STUDENT' | 'TEACHER', email?: string) {
  return {
    ...
    institutionalUsername:
      role === 'STUDENT' ? 'claudio.arrau' : 'camila.rojas',
    ...(email ? { email } : {}),
...
function identityActions(
  overrides: Partial<AccountProvisioningActions> = {},
): AccountProvisioningActions {
  return {
    provisionMembership: vi.fn(async (input) =>
      provisioned(input.role, input.email),
    ),
    inviteMembership: vi.fn(async (membershipId) => ({
      membershipId,
      invitationId: 'invitation-1',
      status: 'PENDING_DELIVERY',
      expiresAt: '2026-08-10T12:00:00Z',
    })),
    createActivationChallenge: vi.fn(async (membershipId) => ({
      membershipId,
      username: 'camila.rojas',
      activationCode: 'shown-once-secret',
      expiresAt: '2026-08-10T12:00:00Z',
    })),
    ...overrides,
  };
...
function academicApi(
  overrides: Partial<AcademicApiClient> = {},
): AcademicApiClient {
  return {
    updateStudent: vi.fn(async (id, input) => ({
      ...edupayStudentNoEmail,
      id,
      email: input.email ?? null,
    })),
    updateTeacher: vi.fn(async (id, input) => ({
      ...teacher,
      id,
      email: input.email ?? null,
    })),
    linkStudentIdentity: vi.fn(async (_id, input) => ({
      ...edupayStudentNoEmail,
      identityUserId: input.identityUserId,
    })),
    linkTeacherIdentity: vi.fn(async (_id, input) => ({
      ...teacher,
      identityUserId: input.identityUserId,
    })),
    ...overrides,
...
describe('AccountProvisioning', () => {
  it('handles an EduPay student without email by requiring email in normal invitation flow and synchronizing it to Academic and Identity', async () => {
    ...
    fireEvent.change(emailInput, {
      target: { value: ' Claudio.Arrau@Piano.CL ' },
    });

    // Submit normal invitation
    fireEvent.click(
      screen.getByRole('button', { name: 'Crear acceso e invitar' }),
    );
    ...
    expect(identity.inviteMembership).toHaveBeenCalledWith(
      'membership-student',
    );
  });
  ...
  it('creates only a STUDENT membership when email was already present, links the user and requests invitation', async () => {
    ...
    render(
      <AccountProvisioning
        ...
        kind="student"
        onLinked={vi.fn()}
        person={studentWithEmail}
      />,
    );
    fireEvent.click(screen.getByRole('button', { name: 'Crear acceso' }));
    expect(screen.getByText('STUDENT')).toBeTruthy();
    expect(screen.getByLabelText('Correo para invitación')).toHaveProperty(
      'value',
      'sofia@example.test',
    );
    fireEvent.click(
    ...
    expect(identity.provisionMembership).toHaveBeenCalledWith({
      institutionalUsername: 'sofia.herrera',
      email: 'sofia@example.test',
      role: 'STUDENT',
    });
    expect(api.linkStudentIdentity).toHaveBeenCalledWith('student-1', {
      identityUserId: 'user-student',
    });
    expect(identity.inviteMembership).toHaveBeenCalledWith(
      'membership-student',
  ...
  it('supports explicit no-email activation-code fallback without sending an invitation email', async () => {
    ...
    expect(identity.provisionMembership).toHaveBeenCalledWith({
      institutionalUsername: 'camila.rojas',
      role: 'TEACHER',
    });
    expect(api.linkTeacherIdentity).toHaveBeenCalledWith('teacher-1', {
      identityUserId: 'user-teacher',
    });
    expect(identity.createActivationChallenge).toHaveBeenCalledWith(
      'membership-teacher',
    );
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Ya lo entregué de forma segura' }),
    );
    await waitFor(() =>
  ...
  it('reports Identity success when the Academic link fails and retries the link without creating another membership', async () => {
    ...
    fireEvent.click(
      screen.getByRole('checkbox', { name: /activar sin correo/i }),
    );
    fireEvent.click(
      screen.getByRole('button', { name: 'Crear acceso con código' }),
    );
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'Reintentar vínculo académico' }),
    );
    expect(await screen.findByText('shown-once-secret')).toBeTruthy();
  ...
  it('handles ambiguous network failure on Identity provisioning by safely receiving the recovered membership on retry and continuing Academic linkage', async () => {
    ...
    expect(
      await screen.findByText(/no pudimos conectar con edupay identity/i),
    ).toBeTruthy();
