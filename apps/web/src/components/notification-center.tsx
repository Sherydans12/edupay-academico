...
export function getSafeNotificationTargetPath(
  ...
  try {
    ...
    if (
      url.origin !== 'https://edupay.academico.invalid' ||
...
export function NotificationCenter({
  ...
  api?: NotificationApiClient | undefined;
}) {
  ...
  const refreshUnreadCount = useCallback(async () => {
    ...
    try {
      ...
      setCountError(
        'No pudimos actualizar tus notificaciones. Puedes reintentarlo.',
      );
    }
  }, [api]);
    ...
    async (cursor?: string) => {
      ...
      try {
        ...
        setNotifications((current) =>
          cursor ? mergeNotifications(current, page.items) : page.items,
        );
        setNextCursor(page.nextCursor);
        setListLoaded(true);
      } catch {
        setListError(
          'No pudimos cargar las notificaciones. Revisa tu conexión e inténtalo nuevamente.',
        );
      } finally {
  ...
  const handleOpenNotification = async (notification: InAppNotification) => {
    ...
    if (!targetPath) {
      setActionError(
        'Este destino no está disponible dentro de EduPay Académico.',
      );
      return;
    }
    ...
    if (
      !notification.readAt &&
      api &&
      !pendingReadIds.current.has(notification.id)
    ) {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      ...
      setActionError(
        'No pudimos marcar todas como leídas. Puedes reintentarlo.',
      );
    } finally {
      setMarkingAll(false);
    }
    ...
    <div className="notification-center">
      ...
      >
        ...
        <span className="sr-only">
          {unreadCount === null
            ? 'El conteo se está actualizando.'
            : unreadCount === 0
        ...
        >
          <header className="notification-panel__header">
            <div>
              ...
              <p>
                {hasUnread
                  ? `${unreadCount ?? 'Algunas'} pendientes de revisar`
                  : 'Todo al día'}
              </p>
            </div>
            <button
          ...
          <div className="notification-panel__actions">
            <span className="notification-panel__count" aria-live="polite">
              {unreadCount === null
                ? 'Actualizando…'
                : `${unreadCount} sin leer`}
            </span>
          ...
          >
            ...
            {!loadingList && listLoaded && notifications.length === 0 ? (
              <div className="notification-empty" role="status">
                ...
                <p>
                  Cuando haya una novedad sobre tus actividades o entregas,
                  aparecerá aquí.
                </p>
              </div>
            ) : null}
