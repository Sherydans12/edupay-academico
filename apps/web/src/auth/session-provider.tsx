...
): { workspace: WorkspaceKind; roleLabel: string } | null {
...
function trustedSession(
  ...
  if (!membership || !mapped) {
    throw new IdentityApiError({
      code: 'FORBIDDEN',
      message: 'No academic workspace is available.',
      status: 403,
    });
  }
  return {
...
export function IdentitySessionProvider({
  ...
  const consumeToken = useCallback(
    async (response: IdentityTokenResponse): Promise<TrustedCurrentSession> => {
  ...
  const value = useMemo<IdentitySessionContextValue>(
    () => ({
      ...
      provisionMembership: async (input) => {
        ...
        return client.provisionMembership(
          context.accessToken,
          context.tenantId,
          {
      ...
      inviteMembership: async (membershipId) => {
        const context = requireManagementContext();
        return client.inviteMembership(
          context.accessToken,
          context.tenantId,
          membershipId,
        );
      ...
      createActivationChallenge: async (membershipId) => {
        const context = requireManagementContext();
        return client.createActivationChallenge(
          context.accessToken,
          context.tenantId,
          membershipId,
        );
  ...
  if (!isPublic && status === 'loading') {
    ...
    content = (
      <main className="session-gate">
        <Alert
          action={
            <Button onClick={() => void retryBootstrap()} variant="secondary">
              Reintentar
            </Button>
          }
    ...
    content = (
      <main
        ...
        className="session-gate"
      >
        <p>Abriendo el inicio de sesión…</p>
      </main>
    );
  }
