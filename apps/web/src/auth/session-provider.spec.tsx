...
function Probe() {
  ...
  return (
    <div>
      ...
      <button
        onClick={() => void Promise.all([auth?.refresh(), auth?.refresh()])}
      >
        refresh twice
      </button>
      ...
      <button onClick={() => void auth?.switchMembership('membership-2')}>
        switch
      </button>
    </div>
  );
...
describe('IdentitySessionProvider', () => {
  it('blocks protected content until refresh-cookie bootstrap resolves and keeps the token out of browser persistence', async () => {
    ...
    expect(
      screen.queryByRole('heading', { name: 'Contenido protegido' }),
    ).toBeNull();
    ...
    expect(
      await screen.findByRole('heading', { name: 'Contenido protegido' }),
    ).toBeTruthy();
  ...
  it('clears the memory session and returns to login after real logout', async () => {
    ...
    await waitFor(() =>
      expect(client.logout).toHaveBeenCalledWith('access-token'),
    );
  ...
  it('consumes login and context-switch replacement tokens and redirects by role', async () => {
    ...
    const client = fakeClient({
      login: vi.fn(async () => ({
        ...token,
        activeMembership: teacherMembership,
      })),
      refresh: vi.fn(async () => {
        throw new IdentityApiError({
          code: 'TOKEN_INVALID',
      ...
      me: vi.fn(async () => ({
        ...profile,
        session: { ...profile.session, activeMembership: teacherMembership },
      })),
    ...
    await waitFor(() =>
      expect(client.switchContext).toHaveBeenCalledWith(
        'access-token',
        'membership-2',
      ),
    );
