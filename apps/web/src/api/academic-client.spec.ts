...
const uploadIntent = {
  ...
  parentId: learningItem.id,
  category: 'STUDENT_SUBMISSION' as const,
  filename: 'trabajo.pdf',
  mimeType: 'application/pdf',
  sizeBytes: 12,
  status: 'RESERVED' as const,
  expiresAt: '2026-08-08T12:15:00+00:00',
  upload: {
    method: 'POST' as const,
    path: `/api/v1/file-upload-intents/${uploadIntentId}/content`,
...
const notification = {
  ...
  body: 'La actividad "Reseña literaria" ya está publicada.',
...
describe('AcademicApiClient', () => {
  it('parses a cursor envelope and sends the Identity access token and request id', async () => {
    ...
    const fetchImpl = vi.fn(
      async (_input: RequestInfo | URL, init?: RequestInit) => {
        void _input;
        capturedInit = init;
        return response({ items: [year], nextCursor: null });
      },
    );
    ...
    await expect(client.listAcademicYears()).resolves.toMatchObject({
      items: [year],
      nextCursor: null,
    });
  ...
  it('refreshes but does not blindly replay a non-idempotent mutation', async () => {
    const localSession = {
      ...session,
      refreshAccessToken: vi.fn(async () => 'replacement-token'),
    };
    const fetchImpl = vi.fn(async () =>
      response(
        {
    ...
    const client = new AcademicApiClient({
      baseUrl: 'http://localhost:3001/api/v1',
      fetchImpl,
      sessionAdapter: localSession,
    });

    await expect(
      client.createStudent({ firstName: 'Sofía', lastName: 'Herrera' }),
    ).rejects.toMatchObject({ code: 'AUTH_REFRESHED_RETRY_REQUIRED' });
  ...
  it('uses idempotent PUT identity-link contracts and can retry them once after refresh', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        response({ ...student, identityUserId: 'identity-user-1' }),
      );
    const localSession = {
      ...session,
      refreshAccessToken: vi.fn(async () => 'replacement-token'),
    ...
    await expect(
      client.linkStudentIdentity(id, { identityUserId: 'identity-user-1' }),
    ).resolves.toMatchObject({ identityUserId: 'identity-user-1' });
    expect(fetchImpl).toHaveBeenCalledTimes(2);
    expect(fetchImpl.mock.calls[0]?.[1]).toMatchObject({
      method: 'PUT',
      body: JSON.stringify({ identityUserId: 'identity-user-1' }),
    });
  });
  ...
  it('fails closed when no Identity adapter can provide an access token', async () => {
    ...
    await expect(client.listAcademicYears()).rejects.toBeInstanceOf(
      UnauthenticatedError,
    );
  ...
  it('reads the Learning route using the shared response contract', async () => {
    const fetchImpl = vi.fn(async () =>
      response({
        courseSubjectId: id,
        units: [{ ...learningUnit, items: [learningItem] }],
      }),
    );
    const client = new AcademicApiClient({
      baseUrl: 'http://localhost:3001/api/v1',
    ...
    await expect(client.getLearningRoute(id)).resolves.toMatchObject({
      courseSubjectId: id,
      units: [{ items: [learningItem] }],
    });
    expect(fetchImpl).toHaveBeenCalledWith(
      `http://localhost:3001/api/v1/course-subjects/${id}/learning`,
      expect.anything(),
    );
  });
  ...
  it('sends typed Learning mutations through the existing request/session boundary', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        response({
          ...
          publishAt: '2099-08-12T20:00:00+00:00',
        }),
      )
    ...
    await client.scheduleLearningItem(learningItem.id, {
      confirmSensitiveChange: false,
      publishAt: '2099-08-12T20:00:00+00:00',
    });
    await client.reorderLearningItems(unitId, {
      orderedIds: [learningItem.id],
    });

    expect(fetchImpl).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining('/learning-units'),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining(`/learning-items/${learningItem.id}/schedule`),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      3,
      expect.stringContaining(`/learning-units/${unitId}/items/reorder`),
      expect.objectContaining({ method: 'POST' }),
    );
  });
  ...
  it('uses the returned upload path and sends one multipart file without embedding bytes in JSON', async () => {
    ...
    const multipartUploadImpl = vi.fn(async (options) => {
      expect(options.url).toBe(
        `http://localhost:3001${uploadIntent.upload.path}`,
      );
    ...
    ).resolves.toMatchObject({ id: uploadIntentId });
    await expect(
      client.completeUploadIntent(uploadIntent, file),
    ).resolves.toMatchObject({ id: storageFile.id });
  ...
  it('calls submission, revision, review, and authorized download endpoints', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(response(submission, 201))
      .mockResolvedValueOnce(
        new Response('private file bytes', {
          status: 200,
          headers: {
            'Content-Disposition': "attachment; filename*=UTF-8''trabajo.pdf",
    ...
    await client.submitLearningItem(learningItem.id, {
      fileObjectIds: [storageFile.id],
    });
    await client.submitSubmissionRevision(submission.id, {
      fileObjectIds: [storageFile.id],
      studentComment: 'Nueva versión',
    });
    await client.reviewSubmissionRevision(
      '00000000-0000-4000-8000-000000000023',
      { action: 'CHANGES_REQUESTED', comment: 'Corrige la conclusión.' },
    );
    const downloaded = await client.downloadFile(storageFile.id);

    expect(fetchImpl).toHaveBeenNthCalledWith(
      1,
      expect.stringContaining(`/learning-items/${learningItem.id}/submission`),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      expect.stringContaining(`/submissions/${submission.id}/revisions`),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      3,
      expect.stringContaining(
        '/submission-revisions/00000000-0000-4000-8000-000000000023/reviews',
      ),
      expect.objectContaining({ method: 'POST' }),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      4,
      expect.stringContaining(`/files/${storageFile.id}/download`),
      expect.objectContaining({ headers: expect.anything() }),
    );
    expect(downloaded.filename).toBe('trabajo.pdf');
    await expect(downloaded.blob.text()).resolves.toContain(
      'private file bytes',
    );
  });
  ...
  it('uses the shared notification contracts for count, cursor pagination, and read mutations', async () => {
    const fetchImpl = vi
      ...
      .mockResolvedValueOnce(
        response({ items: [notification], nextCursor: 'opaque-next-cursor' }),
      )
    ...
    await expect(client.getUnreadNotificationCount()).resolves.toEqual({
      count: 2,
    });
    await expect(
      client.listNotifications('opaque-cursor', 20),
    ...
    ).resolves.toMatchObject({ id: notification.id, readAt: timestamp });
    await expect(client.markAllNotificationsRead()).resolves.toEqual({
      updatedCount: 1,
    });
    ...
    expect(fetchImpl).toHaveBeenNthCalledWith(
      2,
      'http://localhost:3001/api/v1/notifications?cursor=opaque-cursor&limit=20',
      expect.anything(),
    );
    expect(fetchImpl).toHaveBeenNthCalledWith(
      3,
      `http://localhost:3001/api/v1/notifications/${notification.id}/read`,
